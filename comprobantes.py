...
ENV = Environment(
    loader=FileSystemLoader(ROOT / "templates/correos"),
...
TITULOS = {
    "RESERVA": "Tu reserva está confirmada",
    "TORNEO": "Tu equipo ya está inscrito",
    "ESCUELA": "Bienvenido a Súper Chaca",
    "MENSUALIDAD": "Mensualidad registrada",
}
SERVICIOS = {
    "RESERVA": "Reserva de cancha",
    "TORNEO": "Inscripción de torneo",
    "ESCUELA": "Inscripción a Súper Chaca",
    "MENSUALIDAD": "Mensualidad de Súper Chaca",
}
...
def datos_comprobante(conn, orden_id, usuario_id):
    ...
    dato = conn.execute(
        """SELECT o.id AS orden_id,o.tipo,o.descripcion,
        ...
        WHERE o.id=%s AND o.usuario_id=%s AND o.estado='PAGADA' """,
        (orden_id, usuario_id),
    ).fetchone()
    if not dato:
    ...
    if dato["tipo"] == "RESERVA":
        reserva = conn.execute(
            """SELECT r.*,c.nombre AS cancha FROM reservas r
            JOIN canchas c ON c.id=r.cancha_id WHERE r.orden_id=%s""",
            (orden_id,),
        ).fetchone()
        horas = int((reserva["fin"] - reserva["inicio"]).total_seconds() // 3600)
        inicio, fin = reserva["inicio"].astimezone(TZ), reserva["fin"].astimezone(TZ)
        tipo = {"HORA": "Por hora", "EVENTO": "Evento deportivo", "CUMPLEANOS": "Cumpleaños"}[
            reserva["tipo_evento"]
        ]
        dato.update(
            cantidad=f"{horas} h",
            unitario=dato["monto"] / horas,
            detalles=[
                ("Cancha", reserva["cancha"]),
                ("Actividad", tipo),
                ("Día", fecha(inicio.date())),
                ("Horario", f"{inicio:%H:%M} a {fin:%H:%M}"),
        ...
        equipo = conn.execute(
            """SELECT e.nombre,t.nombre AS torneo,t.fecha_inicio,t.max_jugadores
            FROM equipos e JOIN torneos t ON t.id=e.torneo_id WHERE e.orden_id=%s""",
            (orden_id,),
        ).fetchone()
        dato.update(
            detalles=[
                ...
                ("Inicio", fecha(equipo["fecha_inicio"])),
                ("Lista", f"Hasta {equipo['max_jugadores']} jugadores"),
            ],
        ...
        escuela = conn.execute(
            """SELECT sc.alumno,sc.categoria,m.periodo,h.dias,h.inicio,h.fin
            FROM mensualidades m JOIN inscripciones_chaca sc ON sc.id=m.inscripcion_id
            JOIN horarios_chaca h ON h.id=sc.horario_id WHERE m.orden_id=%s""",
            (orden_id,),
        ).fetchone()
        dato.update(
            detalles=[
                ("Alumno", escuela["alumno"]),
                ("Categoría", escuela["categoria"]),
                ("Jornada", escuela["dias"]),
                ("Horario", f"{escuela['inicio']:%H:%M} a {escuela['fin']:%H:%M}"),
...
def ejemplo_comprobante(creado_en):
    ...
    return {
        "orden_id": None,
        "tipo": "TORNEO",
        "descripcion": "Pasochoa Cup · Sexta edición",
...
def contexto_correo(row, base_url):
    ...
    contexto = dict(
        asunto=row["asunto"],
        titulo=row["asunto"],
        preencabezado="Un mensaje de Arena Castell para ti.",
        saludo="",
        parrafos=row["cuerpo"].splitlines(),
        comprobante=None,
        detalles=[],
        url_accion=None,
        texto_accion="",
    ...
    if dato:
        ...
        comprobante = {
            "codigo": (
                f"AC-{dato['pago_id']:06d}" if dato["pago_id"] is not None else "VISTA PREVIA"
            ),
            "fecha": fecha(dato["pagado_en"]),
            "cliente": dato["cliente"],
            "cedula": cedula,
            "servicio": SERVICIOS[dato["tipo"]],
            "descripcion": dato["descripcion"],
            "metodo": METODOS[dato["metodo"]],
            "cantidad": dato["cantidad"],
            "unitario": moneda(dato["unitario"]),
            "total": moneda(dato["monto"]),
            "orden_id": str(dato["orden_id"]) if dato["orden_id"] else "",
        }
        contexto.update(
            titulo="Así se verá tu confirmación" if dato["prueba"] else TITULOS[dato["tipo"]],
            preencabezado=(
                "Correo de prueba: diseño y PDF de Arena Castell."
                if dato["prueba"]
                else f"{comprobante['servicio']} · {comprobante['codigo']} · {comprobante['total']}"
            ),
            saludo=f"Hola, {dato['cliente']}.",
            parrafos=[],
            ...
            prueba=dato["prueba"],
            url_accion=(
                base_url + "/pages/mis_reservas_inscripciones.html"
                if dato["prueba"]
                else base_url + f"/pages/confirmacion.html?orden={dato['orden_id']}"
            ),
        ...
        for link in re.findall(r"https?://[^\s<>]+", row["cuerpo"]):
            ...
            if (
                ...
                and url.path == "/pages/restablecer_contrasena.html"
                and not url.query
                ...
                contexto.update(
                    ...
                    url_accion=link,
                    texto_accion="Cambiar mi contraseña",
                )
                break
    return contexto
...
def crear_pdf(contexto):
    ...
    tinta, gris, linea = (
        ...
        colors.HexColor("#565656"),
        colors.HexColor("#dedede"),
    )
    normal = ParagraphStyle(
        "Normal", fontName="Helvetica", fontSize=10, leading=15, textColor=tinta, spaceAfter=5
    )
    pequeno = ParagraphStyle("Pequeno", parent=normal, fontSize=8, leading=12, textColor=gris)
    titulo = ParagraphStyle(
        "Titulo", parent=normal, fontName="Helvetica-Bold", fontSize=23, leading=28, spaceAfter=12
    )
    ...
    total = ParagraphStyle(
        "Total", parent=derecha, fontName="Helvetica-Bold", fontSize=21, leading=27
    )
    ...
    doc = SimpleDocTemplate(
        ...
        topMargin=133,
        bottomMargin=65,
        title="Comprobante de registro - Arena Castell",
        author="ARENA CASTELL",
        pageCompression=1,
    )
    ...
    def cabecera(canvas, document):
        ...
        canvas.drawImage(
            str(LOGO), 44, A4[1] - 86, width=66, height=62, preserveAspectRatio=True, mask="auto"
        )
    ...
    if contexto["prueba"]:
        story.extend(
            [
                p("VISTA PREVIA - Datos de ejemplo. No corresponde a una operación.", negrita),
                Spacer(1, 8),
            ]
        )
    filas = [
        ("Titular", dato["cliente"]),
        ("Fecha del registro", dato["fecha"] + " (Ecuador)"),
    ...
    tabla = Table(
        [[p(k, pequeno), p(v)] for k, v in filas], colWidths=[126, ancho - 126], hAlign="LEFT"
    )
    tabla.setStyle(
        TableStyle(
            [
    ...
    servicio = Table(
        [
            ...
            [
                [p(dato["servicio"], negrita), p(dato["descripcion"], pequeno)],
                p(dato["cantidad"]),
                p(dato["unitario"], derecha),
                p(dato["total"], derecha),
            ],
        ],
        colWidths=[ancho - 212, 42, 85, 85],
        hAlign="LEFT",
        repeatRows=1,
    )
    servicio.setStyle(
        ...
        [
            KeepTogether(
                [
                    ...
                    p(
                        "IMPORTE REGISTRADO",
                        ParagraphStyle("LabelTotal", parent=pequeno, alignment=TA_RIGHT),
                    ),
                    p(dato["total"], total),
                ]
            ),
            Spacer(1, 18),
            p(contexto["indicacion"]),
            Spacer(1, 10),
        ]
    )
    if dato["orden_id"]:
    ...
    story.append(
        p(
            "Conserva este comprobante para consultar tu registro. La verificación del abono corresponde a la administración.",
            pequeno,
        )
    )
