...
def main():
    ...
    sub.add_parser("update-catalog", help="Actualizar el esquema y catálogo de flyers sin borrar operaciones")
    sub.add_parser("check-db", help="Verificar conexión, tablas y registros sin mostrar credenciales")
    sub.add_parser("create-admin", help="Crear un administrador; la contraseña se solicita sin mostrarla")
    ...
    try:
        if args.command == "check-email":
            ...
            print("Configuración SMTP válida. No se inició sesión ni se envió correo. Ejecuta test-email para comprobar Gmail.")
            return
        if args.command == "test-email":
            correos.enviar_prueba()
            print("El servidor SMTP aceptó el correo de prueba dirigido a tu propia cuenta. Revisa entrada y spam.")
            return
        ...
        with conectar() as conn:
            if args.command == "check-db":
                current = conn.execute("SELECT current_database() AS base, current_user AS usuario").fetchone()
                print(f"Conexión correcta: base {current['base']}, usuario PostgreSQL {current['usuario']}.")
                row = conn.execute("SELECT (SELECT count(*) FROM usuarios) AS usuarios, (SELECT count(*) FROM reservas) AS reservas, (SELECT count(*) FROM pagos) AS pagos").fetchone()
                print(f"Registros: {row['usuarios']} usuarios, {row['reservas']} reservas, {row['pagos']} pagos simulados.")
            elif args.command == "init-db":
                if conn.execute("SELECT to_regclass('public.usuarios') AS existente").fetchone()["existente"]:
                    parser.error("La base ya tiene tablas del proyecto. No se sobrescribió ningún dato.")
                ...
                conn.execute((ROOT/"sql/seed.sql").read_text(encoding="utf8"))
                print("Catálogo actualizado. Se conservaron las órdenes, pagos y listas existentes.")
            elif args.command == "create-admin":
                user = Administrador(input("Nombre: "), input("Correo: "), input("Cédula: "), input("Celular: "))
                ...
                for index, (clase,nombre,email,password) in enumerate([
                    (Administrador,"Administrador Demo","admin@arena.test","CastellAdmin!2026"),
                    (Cliente,"Cliente Demostración","cliente@arena.test","CastellCliente!2026")],1):
                    user = clase(nombre,email,cedula_demo(index),"0990000000")
                    user.set_password(password)
                    if not conn.execute("SELECT id FROM usuarios WHERE email=%s", (email,)).fetchone():
                        insert_user(conn,user)
                print("Cuentas ficticias disponibles. Credenciales en INICIAR.md; solo para demostración local.")
            elif args.command == "outbox":
                rows = conn.execute("""SELECT c.asunto,c.cuerpo,c.creado_en,c.estado_envio,c.ultimo_error FROM correo_salida c
                    JOIN usuarios u ON u.id=c.usuario_id WHERE u.email=%s ORDER BY c.creado_en DESC LIMIT 20""",
                    (args.email.strip().lower(),)).fetchall()
                if not rows:
...
def insert_user(conn,user):
    conn.execute("""INSERT INTO usuarios(nombre,email,cedula,telefono,password_hash,rol)
        VALUES(%s,%s,%s,%s,%s,%s)""", (user.nombre,user.email,user.cedula,user.telefono,user.get_password_hash(),user.rol))
