...
def url_publica():
    value = os.environ.get("PUBLIC_BASE_URL", "").strip() or os.environ.get("APP_ORIGIN", "http://127.0.0.1:8765")
    url = urlsplit(value)
    local = url.hostname in {"localhost", "127.0.0.1", "::1"}
    if (not url.hostname or url.username or url.password or url.query or url.fragment
            or url.path not in {"", "/"} or (url.scheme != "https" and not (local and url.scheme == "http"))):
        raise ConfiguracionCorreoError("PUBLIC_BASE_URL debe ser la dirección HTTPS del sitio, o HTTP local para pruebas.")
    return value.rstrip("/")
...
class ConfiguracionSMTP:
    ...
    def desde_entorno(cls):
        if not habilitado():
            raise ConfiguracionCorreoError("Activa SMTP_ENABLED=true después de configurar tu cuenta.")
        ...
        if (not host or re.search(r"[\s/@]", host) or not 1 <= port <= 65535
                or security not in {"starttls", "ssl"} or not password
                or "\n" in name or "\r" in name):
            raise ConfiguracionCorreoError("Revisa SMTP_HOST, SMTP_PORT, SMTP_SECURITY, SMTP_PASSWORD y MAIL_FROM_NAME.")
        url_publica()
...
def encolar_correo(conn, user_id, email, subject, body, order_id=None, expires=None):
    ...
    conn.execute("""INSERT INTO correo_salida
...
def enviar_smtp(row, config):
    ...
    try:
        ...
        refused = smtp.send_message(message, from_addr=config.usuario, to_addrs=[row["destinatario"]])
        if refused:
...
def procesar_pendientes(limite=10):
    ...
    for _ in range(min(max(limite, 1), 50)):
        with conectar() as conn:
            row = conn.execute("""SELECT c.*,u.email AS email_actual FROM correo_salida c
                JOIN usuarios u ON u.id=c.usuario_id
                WHERE c.estado_envio='PENDIENTE' AND c.intentos<5
                AND c.proximo_intento<=current_timestamp
                ORDER BY c.proximo_intento,c.id LIMIT 1 FOR UPDATE OF c SKIP LOCKED""").fetchone()
            if not row:
                break
            expired = row["vence_en"] and row["vence_en"] <= datetime.now(timezone.utc)
            if expired or row["destinatario"] != row["email_actual"]:
                conn.execute("UPDATE correo_salida SET estado_envio='CANCELADO',ultimo_error=%s WHERE id=%s",
            ...
            try:
                ...
                conn.execute("""UPDATE correo_salida SET intentos=%s,estado_envio=%s,ultimo_error=%s,
                    proximo_intento=current_timestamp+(%s * interval '1 minute') WHERE id=%s""",
                    (attempts, "ERROR" if attempts >= 5 else "PENDIENTE", code, 2 ** (attempts-1), row["id"]))
                logging.warning("Correo %s pendiente de revisión: %s", row["id"], code)
                totals["fallidos"] += 1
            else:
                conn.execute("""UPDATE correo_salida SET estado_envio='ENVIADO',intentos=intentos+1,
                    enviado_en=current_timestamp,ultimo_error=NULL WHERE id=%s""", (row["id"],))
                totals["enviados"] += 1
    return totals
    ...
    def run():
        while not stop.is_set():
            try:
                ...
                logging.warning("Cola de correo pendiente: revisa PostgreSQL y la configuración SMTP.")
            stop.wait(10)
