...
from models import (Cliente, Usuario, ReservaCancha, InscripcionTorneo,
...
def usuario_publico(row):
    return {k: row[k] for k in ("id", "nombre", "email", "cedula", "telefono", "rol")} if row else None


def exigir_usuario(session):
    if not session or not session.get("usuario_id"):
        raise HTTPError(401, "Inicia sesión para continuar. Tus opciones permanecen en esta página.")
...
def nueva_sesion(conn, usuario_id=None, anterior=None):
    ...
    conn.execute("INSERT INTO sesiones(token_hash,usuario_id,csrf_token) VALUES(%s,%s,%s)",
...
def obtener_sesion(conn, token):
    if not token or len(token) > 100:
        return None
    return conn.execute("SELECT * FROM sesiones WHERE token_hash=%s AND vence_en>current_timestamp", (sha(token),)).fetchone()


def limitar_acceso(conn, key):
    # Se confirma independientemente para que los intentos fallidos no se reviertan.
    row = conn.execute("""INSERT INTO intentos_acceso(clave) VALUES(%s)
      ...
      RETURNING intentos""", (sha(key),)).fetchone()
    conn.commit()
    if row["intentos"] > 10:
        raise HTTPError(429, "Se han realizado varios intentos. Espera 15 minutos y vuelve a intentar.")


def registrar(conn, data):
    user = Cliente(data.get("nombre"), data.get("email"), str(data.get("cedula", "")), str(data.get("telefono", "")))
    ...
    return conn.execute("""INSERT INTO usuarios(nombre,email,cedula,telefono,password_hash)
       VALUES(%s,%s,%s,%s,%s) RETURNING *""",
       (user.nombre, user.email, user.cedula, user.telefono, user.get_password_hash())).fetchone()
...
def iniciar_sesion(conn, data):
    ...
    if row:
        ...
        hashlib.pbkdf2_hmac("sha256", str(data.get("password", ""))[:128].encode(), b"arena-login-dummy", 600_000)
        valid = False
    if not valid:
...
def solicitar_restablecimiento(conn, data):
    ...
    if user:
        ...
        conn.execute("INSERT INTO restablecimientos(token_hash,usuario_id,vence_en) VALUES(%s,%s,current_timestamp+interval '30 minutes')",
                     (sha(token), user["id"]))
        conn.execute("""UPDATE correo_salida SET estado_envio='CANCELADO',ultimo_error='ENLACE_REEMPLAZADO'
            WHERE usuario_id=%s AND orden_id IS NULL AND estado_envio='PENDIENTE'""", (user["id"],))
        origin = url_publica()
        encolar_correo(conn, user["id"], user["email"], "Restablecer tu contraseña",
    ...
    return {"message": ("Si el correo está registrado, recibirás un enlace para recuperar tu contraseña. Revisa también la carpeta de spam."
                        if habilitado() else "Si el correo está registrado, tu solicitud quedó registrada. Comunícate con Arena Castell para recuperar tu acceso.")}


def restablecer_password(conn, data):
    if data.get("password") != data.get("confirmacion"):
        raise ErrorValidacion("Las contraseñas no coinciden.")
    reset = conn.execute("""SELECT * FROM restablecimientos WHERE token_hash=%s
       AND NOT usado AND vence_en>current_timestamp FOR UPDATE""", (sha(str(data.get("token", ""))),)).fetchone()
    if not reset:
        raise ErrorValidacion("El enlace ya no es válido. Solicita uno nuevo.")
    row = conn.execute("SELECT * FROM usuarios WHERE id=%s FOR UPDATE", (reset["usuario_id"],)).fetchone()
    user = Usuario.desde_fila(row)
    user.set_password(data.get("password"))
    conn.execute("UPDATE usuarios SET password_hash=%s,session_version=session_version+1 WHERE id=%s",
                 (user.get_password_hash(), user.id))
    conn.execute("UPDATE restablecimientos SET usado=true WHERE usuario_id=%s", (user.id,))
    conn.execute("""UPDATE correo_salida SET estado_envio='CANCELADO',ultimo_error='ENLACE_UTILIZADO'
        WHERE usuario_id=%s AND orden_id IS NULL AND estado_envio='PENDIENTE'""", (user.id,))
...
def catalogo(conn):
    return {"canchas": conn.execute("SELECT * FROM canchas ORDER BY id").fetchall(),
            "horarios_chaca": conn.execute("SELECT * FROM horarios_chaca WHERE activo ORDER BY categoria,id").fetchall(),
            "torneos": conn.execute("""SELECT t.*, t.cupos-count(e.id) AS disponibles FROM torneos t
              LEFT JOIN equipos e ON e.torneo_id=t.id AND e.estado='CONFIRMADO'
              WHERE t.visible GROUP BY t.id ORDER BY t.fecha_inicio""").fetchall(),
            "mensualidad": "50.00", "hoy": datetime.now(TZ).date(), "limite": datetime.now(TZ).date()+timedelta(days=89)}
...
def disponibilidad(conn, data):
    ...
    rows = conn.execute("""SELECT inicio,fin FROM reservas WHERE cancha_id=%s AND estado='CONFIRMADA'
       AND (inicio AT TIME ZONE 'America/Guayaquil')::date=%s""", (cid, day)).fetchall()
    slots = []
    now = datetime.now(TZ)
    for hour in range(8, 24-duration):
        start = datetime.combine(day, datetime.min.time(), TZ) + timedelta(hours=hour)
        end = start + timedelta(hours=duration)
        available = now < start <= now+timedelta(days=90) and not any(start < r["fin"] and end > r["inicio"] for r in rows)
...
def crear_orden(conn, uid, kind, description, service):
    ...
    return conn.execute("""INSERT INTO ordenes(usuario_id,tipo,descripcion,monto)
      VALUES(%s,%s,%s,%s) RETURNING *""", (uid, kind, description, service.calcular_costo())).fetchone()


def reservar(conn, uid, data):
    court = conn.execute("SELECT * FROM canchas WHERE id=%s", (numero(data.get("cancha_id", 1), "Cancha"),)).fetchone()
    if not court:
    ...
    order = crear_orden(conn, uid, "RESERVA", f"{court['nombre']} · {day:%d/%m/%Y} · {hour} · {duration} h", service)
    conn.execute("""INSERT INTO reservas(orden_id,cancha_id,tipo_evento,inicio,fin)
       VALUES(%s,%s,%s,%s,%s)""", (order["id"], court["id"], data.get("tipo_evento"), start, end))
    return {"id": order["id"]}


def inscribir_torneo(conn, uid, data):
    tournament = conn.execute("SELECT * FROM torneos WHERE id=%s FOR UPDATE", (numero(data.get("torneo_id"), "Torneo"),)).fetchone()
    if not tournament or not tournament["abierto"] or tournament["fecha_inicio"] <= datetime.now(TZ).date():
    ...
    order = crear_orden(conn, uid, "TORNEO", f"{tournament['nombre']} · {name}", InscripcionTorneo(tournament["costo"], max_jugadores=tournament["max_jugadores"]))
    conn.execute("INSERT INTO equipos(orden_id,torneo_id,nombre) VALUES(%s,%s,%s)", (order["id"], tournament["id"], name))
    return {"id": order["id"]}
...
def inscribir_escuela(conn, uid, data):
    ...
    order = crear_orden(conn, uid, "ESCUELA", f"Súper Chaca · {name} · {service.categoria}", service)
    schedule = numero(data.get("horario_id"), "Horario de entrenamiento")
    if not conn.execute("SELECT id FROM horarios_chaca WHERE id=%s AND categoria=%s AND activo", (schedule, service.categoria)).fetchone():
        raise ErrorValidacion("Selecciona un horario disponible para la categoría del alumno.")
    inscription = conn.execute("""INSERT INTO inscripciones_chaca(orden_id,alumno,cedula,nacimiento,categoria,horario_id)
      VALUES(%s,%s,%s,%s,%s,%s) RETURNING id""", (order["id"], name, cedula, born, service.categoria, schedule)).fetchone()
    conn.execute("INSERT INTO mensualidades(orden_id,inscripcion_id,periodo) VALUES(%s,%s,%s)",
...
def detalle_orden(conn, uid, oid):
    ...
    order["pago"] = conn.execute("SELECT metodo,referencia,pagado_en,monto,simulado FROM pagos WHERE orden_id=%s", (order["id"],)).fetchone()
    order["reserva"] = conn.execute("SELECT * FROM reservas WHERE orden_id=%s", (order["id"],)).fetchone()
    order["equipo"] = conn.execute("SELECT * FROM equipos WHERE orden_id=%s", (order["id"],)).fetchone()
    order["escuela"] = conn.execute("""SELECT sc.*,m.periodo,h.dias,h.inicio,h.fin FROM mensualidades m JOIN inscripciones_chaca sc
        ON sc.id=m.inscripcion_id JOIN horarios_chaca h ON h.id=sc.horario_id WHERE m.orden_id=%s""", (order["id"],)).fetchone()
    order["correo"] = conn.execute("""SELECT destinatario,estado_envio,enviado_en FROM correo_salida
        WHERE orden_id=%s ORDER BY id DESC LIMIT 1""", (order["id"],)).fetchone()
    return order
...
def pagar(conn, uid, oid, data, *, efectivo_recibido=False):
    ...
    if order["tipo"] in ("ESCUELA", "MENSUALIDAD"):
        ...
        if order["tipo"] == "RESERVA":
            ...
            conn.execute("SELECT c.id FROM canchas c JOIN reservas r ON r.cancha_id=c.id WHERE r.orden_id=%s FOR UPDATE OF c", (order["id"],))
            conn.execute("UPDATE reservas SET estado='CONFIRMADA' WHERE orden_id=%s", (order["id"],))
        elif order["tipo"] == "TORNEO":
            # Bloquear el torneo antes del trigger serializa el último cupo.
            conn.execute("SELECT t.id FROM torneos t JOIN equipos e ON e.torneo_id=t.id WHERE e.orden_id=%s FOR UPDATE OF t", (order["id"],))
            conn.execute("UPDATE equipos SET estado='CONFIRMADO' WHERE orden_id=%s", (order["id"],))
        conn.execute("INSERT INTO pagos(orden_id,monto,metodo,referencia) VALUES(%s,%s,%s,%s)",
    ...
    if receipt["escuela"]:
        sc = receipt["escuela"]
        body += (f"Alumno: {sc['alumno']} · {sc['categoria']}\n"
...
def historial(conn, uid):
    return {
        "ordenes": conn.execute("""SELECT o.*,e.id AS equipo_id FROM ordenes o
            LEFT JOIN equipos e ON e.orden_id=o.id WHERE o.usuario_id=%s ORDER BY o.creado_en DESC""", (uid,)).fetchall(),
        "escuela": conn.execute("""SELECT sc.*,v.mes_actual_pagado,v.ultimo_periodo,v.cuotas_pagadas
            FROM inscripciones_chaca sc JOIN ordenes o ON o.id=sc.orden_id
            JOIN vista_mensualidades_escuela v ON v.inscripcion_id=sc.id WHERE o.usuario_id=%s ORDER BY sc.id""", (uid,)).fetchall(),
        "correos": conn.execute("""SELECT asunto,cuerpo,creado_en,estado_envio,enviado_en FROM correo_salida WHERE usuario_id=%s
            AND orden_id IS NOT NULL ORDER BY creado_en DESC LIMIT 20""", (uid,)).fetchall()
    }


def equipo_usuario(conn, uid, team_id):
    team = conn.execute("""SELECT e.*,t.nombre AS torneo,t.fecha_inicio,t.max_jugadores FROM equipos e JOIN ordenes o ON o.id=e.orden_id
       JOIN torneos t ON t.id=e.torneo_id WHERE e.id=%s AND o.usuario_id=%s""", (numero(team_id, "Equipo"), uid)).fetchone()
    if not team:
...
def lista_equipo(conn, uid, team_id):
    ...
    team["jugadores"] = conn.execute("SELECT id,nombre,cedula,posicion FROM jugadores WHERE equipo_id=%s ORDER BY posicion", (team["id"],)).fetchall()
    return team
...
def agregar_jugador(conn, uid, team_id, data):
    ...
    conn.execute("INSERT INTO jugadores(equipo_id,nombre,cedula) VALUES(%s,%s,%s)", (team["id"], name, cedula))
...
def retirar_jugador(conn, uid, team_id, data):
    ...
    if team["fecha_inicio"] <= datetime.now(TZ).date():
        raise ErrorValidacion("El torneo comenzó; la lista ya está cerrada.")
    conn.execute("DELETE FROM jugadores WHERE id=%s AND equipo_id=%s", (numero(data.get("jugador_id"), "Jugador"), team["id"]))
    return {"message": "Jugador retirado de la lista."}


def renovar_escuela(conn, uid, inscription_id, data):
    inscription = conn.execute("""SELECT sc.* FROM inscripciones_chaca sc JOIN ordenes o ON o.id=sc.orden_id
      WHERE sc.id=%s AND o.usuario_id=%s FOR UPDATE OF sc""", (numero(inscription_id, "Inscripción"), uid)).fetchone()
    ...
    existing = conn.execute("SELECT orden_id FROM mensualidades WHERE inscripcion_id=%s AND periodo=%s", (inscription["id"], period)).fetchone()
    if existing:
        return {"id": existing["orden_id"]}
    # La categoría de ingreso se conserva como dato histórico.
    service = InscripcionSuperChaca(inscription["nacimiento"], inscription["categoria"], inscription["fecha_inscripcion"])
    order = crear_orden(conn, uid, "MENSUALIDAD", f"Súper Chaca · {inscription['alumno']} · {period:%m/%Y}", service)
    conn.execute("INSERT INTO mensualidades(orden_id,inscripcion_id,periodo) VALUES(%s,%s,%s)", (order["id"], inscription["id"], period))
    return {"id": order["id"]}


def actualizar_perfil(conn, uid, data):
    row = conn.execute("SELECT * FROM usuarios WHERE id=%s", (uid,)).fetchone()
    user = Cliente(data.get("nombre"), data.get("email", row["email"]), str(data.get("cedula", row["cedula"])), str(data.get("telefono", "")))
    if (user.email != row["email"] or user.cedula != row["cedula"]) and not Usuario.desde_fila(row).verificar_password(data.get("password_actual", "")):
        raise ErrorValidacion("Para cambiar tu correo o cédula, confirma tu contraseña actual.")
    conn.execute("UPDATE usuarios SET nombre=%s,telefono=%s,email=%s,cedula=%s WHERE id=%s", (user.nombre, user.telefono, user.email, user.cedula, uid))
...
def reportes(conn, uid, data):
    ...
    return {"pagos": payments,
            ...
            "reservas": conn.execute("""SELECT r.id,r.inicio,r.fin,r.tipo_evento,r.estado,
                ...
                ORDER BY r.inicio DESC,r.id DESC""").fetchall(),
            "operaciones": conn.execute("""SELECT o.id,o.creado_en,o.tipo,o.descripcion,o.monto,o.estado,
                u.nombre AS titular,u.email FROM ordenes o JOIN usuarios u ON u.id=o.usuario_id
                ORDER BY o.creado_en DESC,o.id""").fetchall(),
            "escuela": conn.execute("SELECT * FROM vista_mensualidades_escuela ORDER BY alumno").fetchall(),
            "ocupacion": conn.execute("SELECT * FROM vista_ocupacion_cancha ORDER BY mes DESC NULLS LAST").fetchall(),
            "resumen": {"ingresos": sum(p["monto"] for p in payments), "pagos": len(payments),
